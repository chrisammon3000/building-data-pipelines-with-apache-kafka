...
deploy: check-env
	...
	# $(MAKE) elasticsearch.schema.create
...
key-pair.create: ##=> Checks if the key pair already exists and creates it if it does not
	...
	@EC2_KEY_PAIR_NAME="$$(jq -r '.services.elasticsearch.env.ssh_key_name' ${APP_DIR}/config.json)" && \
...
elasticsearch.wait:
	@endpoint=$$(aws ssm get-parameters \
		--names "/${ORGANIZATION}/${APP_NAME}/Ec2ElasticsearchEndpoint" | jq -r '.Parameters[0].Value') && \
	timeout=360 && \
	counter=0 && \
	echo "Waiting for response from Elasticsearch at $$endpoint..." && \
...
elasticsearch.get.endpoint:
	@endpoint=$$(aws ssm get-parameters --names "/${ORGANIZATION}/${APP_NAME}/Ec2ElasticsearchEndpoint" | jq -r '.Parameters[0].Value') && \
	echo "$$endpoint"

elasticsearch.schema.create:
	@bash scripts/create_schema.sh

elasticsearch.schema.delete:
